	...
	IF EXISTS (SELECT p.id_parq FROM parquimetros as p WHERE id_parq=p.id_parq) THEN
		BEGIN
			IF EXISTS (SELECT t.id_tarjeta FROM tarjetas as t WHERE id_tarjeta=t.id_tarjeta) THEN
				BEGIN
					/* tarifa = */ SELECT u.tarifa INTO tarifa FROM ubicaciones as u NATURAL JOIN parquimetros as p WHERE id_parq=p.id_parq;
					/* desc.  = */ SELECT c.descuento INTO descuento FROM tarjetas as t NATURAL JOIN tipos_tarjeta as c WHERE id_tarjeta=t.id_tarjeta;
					
					IF EXISTS (SELECT * FROM estacionamientos as e WHERE id_tarjeta=e.id_tarjeta AND id_parq=e.id_parq AND e.fecha_sal is NULL AND e.hora_sal is NULL ORDER BY fecha_ent,hora_ent DESC LIMIT 1 FOR UPDATE) THEN
						BEGIN #el cliente se va del estacionamiento
							
							#obtenemos el saldo y el tiempo en minuto que estuvo estacionado
							/* saldo   = */ SELECT t.saldo INTO saldo FROM tarjetas as t WHERE id_tarjeta=t.id_tarjeta FOR UPDATE;
							/* minutos = */ SELECT TRUNCATE((TIME_TO_SEC(TIMEDIFF(now(),CONCAT(e.fecha_ent,' ',e.hora_ent)))/60), 2) INTO minutos FROM estacionamientos as e WHERE id_tarjeta=e.id_tarjeta AND id_parq=e.id_parq AND e.fecha_sal is NULL AND e.hora_sal is NULL;
							
							#actualizar para cerra estacionamiento, primero agrego salida, despues modifico el saldo
							UPDATE estacionamientos as e SET e.fecha_sal=now(), e.hora_sal=now() WHERE id_tarjeta=e.id_tarjeta AND id_parq=e.id_parq AND e.fecha_sal is NULL AND e.hora_sal is NULL; 
							UPDATE tarjetas as t set t.saldo=TRUNCATE((saldo-(minutos*tarifa*(1-descuento))),2) WHERE t.id_tarjeta=id_tarjeta;
							
							#Resultado
							SELECT 'Cierre' as Operacion, TRUNCATE(minutos,2) as 'Tiempo transcurrido (min.)', TRUNCATE((saldo-(minutos*tarifa*(1-descuento))),2) as 'Saldo actualizado';	
